...
output_dir <- "/OSM/CBR/AG_WHEATTEMP/work/avgTemp/"
...
#minVal <- round(min(all_df$avgTemp), 0) - 2
#maxVal <- round(max(all_df$avgTemp), 0) + 2

#maxdf <- all_df[all_df$avgTemp >= 30,]
#mindf <- all_df[all_df$avgTemp <= 19,]
...
ggplot() + 
    ...
    geom_sf(data = points_sf, aes(colour=avgTemp), alpha=0.7, show.legend = "point") +
    scale_colour_gradientn(colours=colPal, limits=c(10, 26)) +
