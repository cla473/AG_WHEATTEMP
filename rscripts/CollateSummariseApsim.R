...
option_list = list(
    make_option(c("-f", "--filename"), type="string", help="The Name of the Simulation (.RData) file.")
);
...
basefilename = paste0(outPath, "/", cultivar, ".csv") 
...
dfSeasonDates <- fulldf %>%
    ...
    mutate(seasonDayCount = seasonStartDOY - seasonEndDOY)%>%
    ungroup() %>%
...
dfSummary <- dfSummary %>% 
    mutate(calcPD = cumPARIO / avgTemp,
           calcPQvpd = cumPARIO / (avgTemp * avgVPD),
           calcPQvpdfdr = cumPARIO / (avgTemp * avgVPD * avgFDR)) %>% 
